...
export const LapChart = (props) => {
  ...
  useEffect(() => {
    ...
    sortedDriverAcronyms.forEach((acronym, index) => {
      ...
      initialVisibility[acronym] = driverCode
        ? acronym === driverCode
        : index < 3;
    });
  ...
  const prepareChartData = () => {
    const driverAcronyms = [
      ...new Set(laps.map((lap) => driversDetails[lap.driver_number])),
    ];
    const lapNumbers = [...new Set(laps.map((lap) => lap.lap_number))].sort(
      (a, b) => a - b
    );
      ...
      .map((lapNumber) => {
        ...
        driverAcronyms.forEach((acronym) => {
          if (!driverCode || acronym === driverCode) {
            const lapForDriver = laps.find(
              (lap) =>
                lap.lap_number === lapNumber &&
                driversDetails[lap.driver_number] === acronym
            );
            if (lapForDriver) {
              const lapDurationInSeconds = parseFloat(
                lapForDriver.lap_duration
              );
              if (
                !isNaN(lapDurationInSeconds) &&
                lapDurationInSeconds > 0 &&
                lapDurationInSeconds <= 180 &&
                lapDurationInSeconds >= lowerBound &&
                lapDurationInSeconds <= upperBound
              ) {
                // Convert lap duration from seconds to minutes and exclude outliers
                lapDataForAllDrivers[acronym] = (
                  lapDurationInSeconds / 60
                ).toFixed(5);
              }
            }
          }
        ...
        return Object.keys(lapDataForAllDrivers).length > 1
          ? lapDataForAllDrivers
          : null;
      })
  ...
  const minLapDuration = Math.min(
    ...chartData.flatMap((lap) => Object.values(lap).slice(1))
  );
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-4 sm:max-w-[80%] sm:mx-auto">
            {sortedDriverAcronyms.map((acronym, index) => (
              <button
                ...
                style={{
                  backgroundColor: driverVisibility[acronym]
                    ? `${newDriversColor[acronym]}`
                    : "#333333",
                }}
              >
                {acronym}
