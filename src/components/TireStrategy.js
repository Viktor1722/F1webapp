...
export const TireStrategy = (props) => {
  ...
  const transformedData = drivers.map((driver) => {
    ...
    driver.tires.forEach((tire, index) => {
      const lapEndValue =
        index === 0 ? tire.lap_end : tire.lap_end - previousLapEnd;
  ...
  const sortedTransformedData = sortedDriverAcronyms
    .map((acronym) => {
      return filteredTransformedData.find(
        (driverData) => driverData.acronym === acronym
      );
    })
        ...
        <div className="custom-tooltip">
          ...
          {payload.map((entry, index) => (
            <p
              key={index}
              style={{
                color: tireTypeClasses[entry.name.replace(/[0-9]/g, "")],
              }}
            >
              {capitalizeFirstLetter(entry.name.replace(/[0-9]/g, ""))}:{" "}
              {entry.value}
            </p>
          ))}
        </div>
      ...
      <div className=" sm:p-32 mb-16 ">
            ...
            {tireKeys
              ...
              .map((key) => (
                <>
                  ...
                  <Bar
                    className={`tire-compound tire-compound--${key.replace(
                      /[0-9]/g,
                      ""
                    )}`}
                    dataKey={key}
        ...
        <div
          ...
          style={{
            background:
              driverCode &&
              `radial-gradient(circle at center, #${driverColor} 0%, #${driverColor}00 70%)`,
          }}
        />
        <div
